...
from ...models.care import Appointment, Practitioner, PractitionerAvailabilityRule
...
async def availability(facility_uuid: UUID, practitioner_uuid: UUID, target_date: date = Query(alias="date"), account: Annotated[UserAccount, Depends(get_current_identity_account)] = ..., db: Annotated[AsyncSession, Depends(async_get_db)] = ..., duration_minutes: int = Query(default=30, ge=5, le=240), enforce_future: bool = False) -> dict[str, Any]:
    ...
    appointments = (await db.scalars(select(Appointment).where(Appointment.facility_id == facility_uuid, Appointment.practitioner_id == practitioner_uuid, Appointment.status.in_(["booked", "confirmed", "checked_in", "in_consultation"]), Appointment.scheduled_start >= datetime.combine(target_date, time.min), Appointment.scheduled_start < datetime.combine(target_date + timedelta(days=1), time.min)))).all()
    slots = []
    current = datetime.combine(target_date, start)
    closing = datetime.combine(target_date, end)
    ...
    while current + timedelta(minutes=duration_minutes) <= closing:
        ...
        past = enforce_future and target_date == date.today() and current < now
    ...
    return {"success": True, "data": {"facilityId": str(facility_uuid), "facilityUuid": str(facility_uuid), "practitionerId": str(practitioner_uuid), "practitionerUuid": str(practitioner_uuid), "date": target_date.isoformat(), "timezone": "Asia/Kolkata", "durationMinutes": duration_minutes, "operatingHours": {"start": start.strftime("%H:%M"), "end": end.strftime("%H:%M")}, "nextAvailableSlot": next_slot, "slots": slots}, "meta": {}}
